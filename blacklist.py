...
def main():
    ftpsodex = connect("192.168.1.8", "ftpuser", "MeiyouMima123", "blacklist")
    logging.info("begin to download blacklist file")
    ...
    ftpremote = connect("192.168.1.8", "ftpuser", "MeiyouMima123", "trans")
    logging.info("begin to upload file to remote path")
    if upload(ftpremote, FILENAME):
        logging.info("upload blacklist file successful")
        pass
    else:
        logging.error("upload blacklist failed")
